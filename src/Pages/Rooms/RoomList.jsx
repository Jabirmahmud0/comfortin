        ...
        <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
            <div className="container mx-auto px-4 py-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {rooms.map((room) => (
                        <div key={room.id} className={`border rounded-lg shadow-lg overflow-hidden transition-all duration-300 hover:shadow-xl ${
                            ...
                            <div className="p-6">
                                ...
                                <div className={`text-sm mb-4 space-y-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                                    <p><span className="font-medium">Size:</span> {room.RoomSize}</p>
                                    <p><span className="font-medium">Status:</span> {room.Availability}</p>
                                </div>
                                <div className="flex justify-between items-center">
                                    ...
                                    <Link to={`/room-details/${room.id}`}>
                                        <button className={`px-4 py-2 rounded-lg font-medium transition-colors ${
                                            isDarkMode 
                                                ? 'bg-blue-600 text-white hover:bg-blue-700' 
                                                : 'bg-[#4D869C] text-white hover:bg-[#3a6b7a]'
                                        }`}>
                                            Book Now
                                        </button>
                                    </Link>
                ...
                {rooms.length === 0 && (
                    <div className={`text-center py-12 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <h3 className="text-xl mb-4">No rooms found</h3>
                        <p>Try adjusting your price filters or check back later.</p>
                    </div>
                )}
            </div>
        </div>
